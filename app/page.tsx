...
// ─── STEP DEFINITIONS ──────────────────────────────────────────────
const STEP_IDS = [
  "prerequisites", "install", "verify", "auth",
  "init", "build", "deploy", "logs", "chat", "faq",
];
...
export default function Home() {
  ...
  const complete = (id: string) => setCompletedSteps((prev) => ({ ...prev, [id]: true }));
  const uncomplete = (id: string) => setCompletedSteps((prev) => ({ ...prev, [id]: false }));

  const isCompleted = (id: string) => completedSteps[id] ?? false;

  const stepsWithStatus = STEP_IDS.map((id, i) => ({
    id,
    label: ["Prerequisites", "Installation", "Verify Install", "Authentication", "Project Init", "Build Firmware", "Deploy", "Log Analysis", "Chat Mode", "Troubleshooting"][i],
    ...
    <div className="min-h-screen bg-[#0a0e13]">
      ...
      <header className="relative border-b border-terminal-border overflow-hidden min-h-[500px] flex flex-col">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-20 text-center flex-1 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-terminal-comment/60 max-w-lg mx-auto mb-10" style={{ animation: "slideUp 0.7s ease-out 0.5s both" }}>
            Read each section, then complete the interactive challenge to unlock the next step.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-2 mb-10" style={{ animation: "slideUp 0.7s ease-out 0.6s both" }}>
            {[
              ...
              <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-terminal-border bg-[#0d1117]/80 text-xs text-terminal-comment">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3" style={{ animation: "slideUp 0.7s ease-out 0.7s both" }}>
            <a href="https://fwauto.ai" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-6 py-3 rounded-xl bg-terminal-green text-black text-sm font-bold hover:bg-terminal-green/90 transition-all active:scale-95">
      ...
      <section className="border-b border-terminal-border bg-[#0d1117]/60">
        <div className="max-w-7xl mx-auto px-6 py-16">
          <div className="text-center mb-10">
            ...
            <p className="text-terminal-comment text-sm mt-2">Press <span className="text-terminal-green font-semibold">Run Demo</span> to watch each flow in a real terminal.</p>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="lg:w-60 shrink-0">
            <ProgressTracker steps={stepsWithStatus} xp={0} unlockedAchievements={[]} />
          </aside>
          ...
          <div className="flex-1 min-w-0 space-y-6">
            ...
            <section id="prerequisites">
              <StepCard
                ...
                badge="Required" badgeColor="yellow"
                isCompleted={isCompleted("prerequisites")}
                onComplete={(c) => c ? complete("prerequisites") : uncomplete("prerequisites")}
                challengeLabel="🧠 Quick Knowledge Check"
                challenge={
                  <QuizChallenge
                    question="Which two tools must be installed before running the FWAuto installer?"
                    options={[
                      { id: "a", label: "Python and pip", explanation: "FWAuto uses uv, not plain pip." },
                      { id: "b", label: "uv and Node.js", correct: true, explanation: "Correct! uv is the Python package manager and Node.js provides npm for the AI CLI tools." },
                      { id: "c", label: "Docker and Git", explanation: "FWAuto doesn't require Docker or Git to install." },
                      { id: "d", label: "Node.js and Go", explanation: "Go isn't needed. FWAuto uses uv for Python tooling." },
                    ]}
              ...
              >
                <div className="rounded-xl border border-terminal-border overflow-hidden">
                  ...
                  <div className="divide-y divide-terminal-border/50">
                    ...
                    {[
                      { tool: "FWAuto Account", version: "Registered", purpose: "Create free account at FWAuto Dashboard" },
                ...
                <Callout type="warning" title="Windows: Node.js needs MSI installer">
                  On Windows, install Node.js via the <strong>.msi installer</strong> from nodejs.org and run PowerShell as Administrator. See Appendix A.
                </Callout>
              </StepCard>
            </section>
            ...
            <section id="install">
              <StepCard
                ...
                badge="One Command" badgeColor="green"
                isCompleted={isCompleted("install")}
                onComplete={(c) => c ? complete("install") : uncomplete("install")}
              ...
              >
                <OsTabSelector content={{
                  linux: (
                    <div className="space-y-3">
                      ...
                      <Callout type="tip" title="What does this do?">
                        Downloads the install script and pipes it directly to <code>bash</code>. Installs both <code>fwauto</code> and the AI CLI tools.
                      </Callout>
                    </div>
                  ),
                  macos: (
                  ...
                  windows: (
                    <div className="space-y-3">
                      <p className="text-terminal-comment text-sm">Open <strong className="text-terminal-text">PowerShell as Administrator</strong>:</p>
                      <CodeBlock code={`powershell -ExecutionPolicy ByPass -c "irm https://fwauto.ai/install.ps1 | iex"`} title="PowerShell (Admin)" />
                      <Callout type="warning">Must be run from an elevated (Admin) PowerShell session.</Callout>
                    </div>
                  ),
                }} />
                <p className="text-sm text-terminal-comment">Watch the installer:</p>
            ...
            <section id="verify">
              <StepCard
                ...
                onComplete={(c) => c ? complete("verify") : uncomplete("verify")}
                challengeLabel="⌨️ Type the Command"
                challenge={
                  <TerminalTyping
                    prompt="fwauto --help"
              ...
              >
                ...
                <CodeBlock code={`FWAUTO  v0.9.0\nUsage: fwauto [OPTIONS] COMMAND [ARGS]...\n\n🚀 STM32 Firmware Automation Tool with AI\n\nCommands:\n  dashboard, build, deploy, log, auth, config, rag`} showPrompt={false} title="Expected Output" />
                <Callout type="success" title="If you see the banner, you're good to go!">
            ...
            <section id="auth">
              <StepCard
                ...
                badge="Google OAuth" badgeColor="blue"
                ...
                challenge={
                  <QuizChallenge
                    ...
                    options={[
                      { id: "a", label: "Copy the URL and paste it into the terminal", explanation: "No, you interact with the browser directly — no copy-pasting needed." },
                      { id: "b", label: "Sign in with the Google account you registered with on the FWAuto Dashboard, then grant access", correct: true, explanation: "Correct! Select your registered Google account and click Continue to grant access." },
                      { id: "c", label: "Close the browser and re-run the command", explanation: "No — wait for the browser flow to complete first." },
                      { id: "d", label: "Enter your FWAuto password in the browser", explanation: "FWAuto uses Google OAuth — there's no separate FWAuto password." },
                    ]}
              ...
              >
                ...
                <div className="space-y-3">
                  {[
                    { num: "1", title: "Choose your Google account", desc: "Select the account you registered on the FWAuto Dashboard." },
                    { num: "2", title: "Grant access", desc: 'FWAuto requests name, photo, and email. Click "Continue".' },
                    { num: "3", title: "Login successful", desc: 'Browser shows "Login Successful!". Close it and return to the terminal.' },
                  ].map((s) => (
                    <div key={s.num} className="flex gap-4 p-4 rounded-xl border border-terminal-border/50 bg-[#161b22]">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-terminal-comment">{s.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <StepCard
                ...
                challenge={
                  <DragMatch
                    ...
                    targets={[
                      { id: "t1", label: "Choose how firmware gets onto the device: network (SSH), serial port, or custom command.", matchId: "deploy" },
                      { id: "t2", label: "Enter the path to your firmware SDK (e.g. /home/user/ATK-AM62x-SDK).", matchId: "sdk" },
                      { id: "t3", label: "Choose your build method: Makefile or custom shell command.", matchId: "build" },
                    ]}
                    onSuccess={() => complete("init")}
                  />
                }
              >
                <Callout type="info">Navigate into your firmware project directory before running this.</Callout>
                <CodeBlock code={`cd /path/to/your/firmware-project\nfwauto build`} />
                <div className="space-y-3">
                  {[
                    { step: "[1/3] SDK Configuration", desc: "Enter the path to your firmware SDK. Press Enter to accept the default.", example: "SDK path [/opt/ti-sdk]: /home/user/ATK-AM62x-SDK" },
                    { step: "[2/3] Build Configuration", desc: "Select '1' for Makefile or '2' for a custom command.", example: "Build type:\n  1. makefile\n  2. command\nSelect [1]:" },
                    { step: "[3/3] Deploy Configuration", desc: "Select 'network' (SSH), 'serial', or 'command'.", example: "Deploy type:\n  1. network\n  2. serial\n  3. command\nSelect [1]:" },
                  ].map((item, i) => (
                    <div key={i} className="rounded-xl border border-terminal-border/50 overflow-hidden">
                      ...
                      <div className="p-4 space-y-2">
                        <p className="text-sm text-terminal-comment">{item.desc}</p>
                        <CodeBlock code={item.example} showPrompt={false} className="text-xs" />
                      </div>
                    </div>
                  ))}
                </div>
                <CodeBlock code={`.fwauto/\n├── config.toml      # Project configuration\n├── build/\n│   └── Makefile     # Build script (customize this!)\n└── logs/            # UART log storage`} showPrompt={false} title="Generated Structure" />
                <Callout type="warning">The generated Makefile is a template. Customize it for your project before building.</Callout>
              </StepCard>
            </section>
            ...
            <section id="build">
              <StepCard
                ...
                badge="AI Auto-Repair" badgeColor="purple"
                isCompleted={isCompleted("build")}
                onComplete={(c) => c ? complete("build") : uncomplete("build")}
                challengeLabel="🔢 Put the Build Flow in Order"
                challenge={
                  <SequenceChallenge
                    question="When a build fails, what order does FWAuto follow?"
                    items={[
                      { id: "apply", label: "Apply the AI-suggested fix to the source code", sublabel: "AI patches the file automatically" },
                      { id: "fail", label: "Build fails with a compilation error", sublabel: "Non-zero return code from make" },
                      { id: "retry", label: "Retry the build (up to 3 attempts)", sublabel: "fwauto build retries automatically" },
                      { id: "ai", label: "AI analyzes the error output", sublabel: "stdout/stderr sent to AI" },
                      { id: "success", label: "Build succeeds ✓", sublabel: "Return code 0" },
                    ]}
              ...
              >
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="p-4 rounded-xl border border-green-800 bg-green-950/20">
                    ...
                    <p className="text-sm text-terminal-comment">Output placed in the directory configured in your Makefile.</p>
                  </div>
                  <div className="p-4 rounded-xl border border-yellow-800 bg-yellow-950/20">
                    ...
                    <p className="text-sm text-terminal-comment">AI captures the error, suggests a fix, applies it, retries. Up to 3 attempts.</p>
                  </div>
                </div>
                <TerminalDemo title="Build & AI Repair Demo" script={BUILD_DEMO} />
                <Callout type="info">AI repair only triggers on compilation errors — not config errors like a missing Makefile.</Callout>
              </StepCard>
            </section>
            ...
            <section id="deploy">
              <StepCard
                ...
                onComplete={(c) => c ? complete("deploy") : uncomplete("deploy")}
                ...
                challenge={
                  <FillConfig
                    ...
                    lines={[
                      "[project]",
                      { id: "name", placeholder: "your-project", answer: "my-firmware", hint: "Any project name works — try 'my-firmware'" },
                      "",
                      "[deploy]",
                      { id: "type", placeholder: "network / serial / command", answer: "network", hint: "For SSH deploy, the type is 'network'" },
                      "",
                      "[deployment]",
                      { id: "ip", placeholder: "board IP address", answer: "192.168.1.100", hint: "Try '192.168.1.100'" },
                      { id: "user", placeholder: "login user on the board", answer: "root", hint: "Embedded Linux boards usually log in as 'root'" },
                      { id: "path", placeholder: "path on the board", answer: "/home/root", hint: "Try '/home/root'" },
                    ]}
              ...
              >
                <div className="space-y-3">
                  <div>
                    <p className="text-sm text-terminal-comment mb-2">Basic deploy:</p>
                    <CodeBlock code={`fwauto deploy`} />
                  </div>
                  <div>
                    <p className="text-sm text-terminal-comment mb-2">Deploy with startup args:</p>
                    <CodeBlock code={`fwauto deploy --binary-args "on"`} />
                  </div>
                </div>
                <Callout type="tip" title="Network deploy (SSH)">Configure <code className="text-purple-300">board_ip</code>, <code className="text-purple-300">board_user</code>, and <code className="text-purple-300">deploy_path</code> in <code>config.toml</code>.</Callout>
              </StepCard>
            </section>
            ...
            <section id="logs">
              <StepCard
                ...
                badge="AI Analysis" badgeColor="blue"
              ...
              >
                <div className="space-y-3">
                  <div>
                    <p className="text-sm text-terminal-comment mb-2">Analyze the most recent log:</p>
                    <CodeBlock code={`fwauto log "Are there any errors?"`} />
                  </div>
                  <div>
                    <p className="text-sm text-terminal-comment mb-2">Analyze a specific file:</p>
                    <CodeBlock code={`fwauto log "Summarize this log" --log-path /path/to/uart.log`} />
                  </div>
                </div>
                <div className="grid sm:grid-cols-2 gap-2">
                  {['"Are there any errors?"', '"What happened during boot?"', '"Why did the device reset?"', '"Summarize this log"'].map((q, i) => (
                    <div key={i} className="px-3 py-2 rounded-lg border border-terminal-border bg-[#161b22] text-sm font-mono text-terminal-green">{q}</div>
                  ))}
                </div>
              </StepCard>
            </section>
            ...
            <section id="chat">
              <StepCard
                ...
                challenge={
                  <DragMatch
                    ...
                    targets={[
                      { id: "t1", label: "View or change project settings like language", matchId: "config" },
                      { id: "t2", label: "Compile the firmware project", matchId: "build" },
                      { id: "t3", label: "Quit the interactive chat session", matchId: "exit" },
                      { id: "t4", label: "Flash firmware onto the connected device", matchId: "deploy" },
                      { id: "t5", label: "Analyze the latest UART log with AI", matchId: "log" },
                    ]}
              ...
              >
                <div className="space-y-3">
                  ...
                  <div>
                    <p className="text-sm text-terminal-comment mb-2">Start with a message:</p>
                    <CodeBlock code={`fwauto "Make the LED blink faster"`} />
                  </div>
                </div>
                <CommandTable commands={slashCommands} title="All Slash Commands" />
                <Callout type="tip">Switch AI to Traditional Chinese: <code className="text-purple-300">/config set project.language zh-TW</code></Callout>
              </StepCard>
            </section>
              ...
              <StepCard
                ...
                challenge={
                  <QuizChallenge
                    ...
                    multiSelect
                    options={[
                      { id: "a", label: "src/main.c:55: error: 'LED_PIN' undeclared", correct: true, explanation: "✓ This is a real compilation error — AI repair will activate." },
                      { id: "b", label: "make: .fwauto/build/Makefile: No such file or directory", explanation: "✗ This is a configuration error (missing Makefile), not a compilation error." },
                      { id: "c", label: "src/uart.c:12: error: implicit declaration of function 'UART_Init'", correct: true, explanation: "✓ Compilation error — AI repair will activate." },
                      { id: "d", label: "fwauto: SDK path not found: /opt/ti-sdk", explanation: "✗ This is a configuration error — SDK path is wrong." },
                    ]}
            ...
            <div id="windows-prereqs" className="rounded-2xl border border-terminal-border bg-[#0d1117] p-6 space-y-5">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-terminal-comment mt-2">Close and reopen PowerShell as Administrator after running this.</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-terminal-text mb-2">Step 2: Install Node.js via .msi</p>
                  <p className="text-sm text-terminal-comment mb-2">Download from <a href="https://nodejs.org/en/download" target="_blank" rel="noopener noreferrer" className="text-terminal-blue underline">nodejs.org</a>. Version ≥ 20. Ensure "Add to PATH" is checked.</p>
                </div>
                <div>
            ...
            <div className="rounded-2xl border border-terminal-border bg-[#0d1117] p-6 space-y-5">
              ...
              <CommandTable title="Required Rules" commands={[
                { cmd: "build (target)", description: "Must exist. FWAuto calls 'make build' by default." },
                { cmd: "Return code 0", description: "Non-zero = failure. May trigger AI repair." },
                { cmd: "stdout/stderr", description: "Captured and sent to AI for analysis." },
              ]} />
            </div>
            ...
            <div className="rounded-2xl border border-terminal-green/30 bg-terminal-green/5 p-8 text-center">
              ...
              <p className="text-terminal-comment text-sm max-w-md mx-auto mb-6">All 10 steps covered. Head to the FWAuto Dashboard to manage your projects.</p>
              <a href="https://fwauto.ai" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-terminal-green text-black text-sm font-bold hover:bg-terminal-green/90 transition-all active:scale-95">
      ...
      <footer className="border-t border-terminal-border mt-20 py-8">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <a href="https://fwauto.ai" target="_blank" rel="noopener noreferrer" className="text-xs text-terminal-comment font-mono hover:text-terminal-green transition-colors">fwauto.ai</a>
        </div>
      </footer>
    </div>
  );
